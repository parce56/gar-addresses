...
class AddrObj extends Model {
    ...
    async upsert(data) {
        const query = `
            INSERT INTO "${this.schema}".addrobjs (
                id, objectid, objectguid, changeid, name, typename, level,
        ...
        const values = [
            ...
            data.name, data.typename, data.level, data.opertypeid, 
            data.previd, data.nextid, data.updatedate, data.startdate, 
            data.enddate, data.isactual, data.isactive
        ]
        try {
    ...
    async bulkUpsert(items) {
        ...
        validItems.forEach((item, index) => {
            ...
            values.push(
                ...
                item.typename !== undefined ? item.typename : null,
